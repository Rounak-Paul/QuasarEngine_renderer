...
namespace Quasar {
	...
	Pipeline::~Pipeline()
	{
		vkDestroyShaderModule(qsDevice.device(), vertShaderModule, nullptr);
		vkDestroyShaderModule(qsDevice.device(), fragShaderModule, nullptr);
		vkDestroyPipeline(qsDevice.device(), graphicsPipeline, nullptr);
	}
		...
		const PipelineConfigInfo& configInfo)
	{
		...
		{
			QS_CORE_ERROR("failed to create Graphics Pipeline");
		}
	}
