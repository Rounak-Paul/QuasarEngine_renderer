...
namespace Quasar {
	...
	void PointLightSystem::update(FrameInfo& frameInfo, GlobalUbo& ubo) {
		...
		for (auto& kv : frameInfo.gameObjects) {
			...
			obj.transform.translate = glm::vec3(rotateLight * glm::vec4(obj.transform.translate, 1.f));

			// copy light to ubo
			ubo.pointLights[lightIndex].position = glm::vec4(obj.transform.translate, 1.f);
	...
	void PointLightSystem::Render(FrameInfo& frameInfo)
	{
		...
		for (auto& kv : frameInfo.gameObjects) {
			...
			push.position = glm::vec4(obj.transform.translate, 1.f);
